...
from django.core.mail import send_mail #### Uses SMTP -> (Full form and port number) && and HTTP
import csv
...
def createDoctor(request):
    try:
        doc_name = request.GET.get('Name1')  #name is variable id from html
        ...
        try:
            user_exists = False
            posts = login.objects.all() # databse.objects.all() -> to get all the rows from the login table.
            for post in posts:
                if(doc_mob_no == post.loginId):
                    print("mobile already exist")
                    user_exists = True
            if(not user_exists):
                        doc = Doctor() # to initiate with Doctor table
                        ...
                        try:
                            ...
                            send_mail( subject, message, email_from, recipient_list ) #send mail parametere
     ...
     try:
        ...
        try:
            ...
            if(not user_exists):
                        ...
                        enc_uname2 = hashlib.sha256(rec_mob_no.encode("utf-8")).hexdigest() #rec_mob_no gets encoded
...
def encrypt_val(clear_text):
    ...
    return ciphertext
    '''
    encryption_suite = AES.new(MASTER_KEY, AES.MODE_CFB, 'This is an IV456') #master_key at line 7
    cipher_text = encryption_suite.encrypt(clear_text)
    return cipher_text
